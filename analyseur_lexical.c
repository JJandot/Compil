...
char *tableMotsClefs[] = {
  "si","alors","sinon","tantque", "faire","entier","retour","lire", "ecrire", "incr" 
};
...
int codeOperateursEtSymboles[] = {POINT_VIRGULE, PLUS, MOINS, FOIS, DIVISE, 
																	PARENTHESE_OUVRANTE, PARENTHESE_FERMANTE, 
																	CROCHET_OUVRANT, CROCHET_FERMANT, 
																	ACCOLADE_OUVRANTE, ACCOLADE_FERMANTE, EGAL,
																	INFERIEUR, ET, OU, NON, VIRGULE
																	};

char *tableOperateursEtSymboles[] = {
	';', '+', '-', '*', '/', '(', ')', '[', ']', '{', '}', '=', '<', '&', '|', '!', ','
};
...
int yylex(void)
{
  ...
  yytext[yyleng = 0] = '\0';
	...
	if(i == -1)
		return FIN;
	c = lireCar();
	for(i = 0; i < nbOperateursEtSymboles; ++i){
		if(c == tableOperateursEtSymboles[i]){
			return codeOperateursEtSymboles[i];	
		}
	}
	int exit = 1;
	while(exit){
	...
	for(j = 0; j < nbMotsClefs; ++j){
		if(strcmp(yytext, tableMotsClefs[j]) == 0){
			return codeMotsClefs[j];
		}
	}
	
	for(j = 0; j < sizeof(yytext); ++j){
		if(!is_num(yytext[j]))
			break;
		return NOMBRE;
	}
...
void analyseur_lexical(FILE *yyin) {
  int uniteCourante;
  char nom[100];
  char valeur[100];  
  do {
    uniteCourante = yylex();
  //  printf("c : %d", uniteCourante);
